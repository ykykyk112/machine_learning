...
def train_eval_model_gpu(model, epoch, device, train_loader, test_loader, cam_mode, save_path = None) :
    ...
    for i in range(epoch) :
        ...
        for idx, (train_data, train_target) in enumerate(train_loader) :
            ...
            sum_loss = (t_loss*(1.0) + b_loss*(0.5) + e_loss*(0.5))
            sum_loss.backward()
            ...
            train_loss += t_loss.item()
            train_acc += (torch.sum(pred == train_target.data)*(100.0 / batch_size)).item()
            boundary_acc += (torch.sum(boundary_pred == train_target.data)*(100.0 / batch_size)).item()
            ensemble_acc += (torch.sum(ensemble_pred == train_target.data)*(100.0 / batch_size)).item()
            print('-----------------------------------')
            print(torch.sum(pred == train_target.data)*(100.0 / batch_size).item())
            print(torch.sum(boundary_pred == train_target.data)*(100.0 / batch_size).item())
            print(torch.sum(ensemble_pred == train_target.data)*(100.0 / batch_size).item())
            print('-----------------------------------')
