...
def train_eval_model_gpu(model, epoch, device, train_loader, test_loader, cam_mode, save_path = None) :
    ...
    for i in range(epoch) :
        ...
        for idx, (train_data, train_target) in enumerate(train_loader) :
            ...
            print('-----------------------------------')
            print(torch.sum(pred == train_target.data).mul_(100.0 / batch_size).item())
            print(torch.sum(boundary_pred == train_target.data).mul_(100.0 / batch_size).item())
            print(torch.sum(ensemble_pred == train_target.data).mul_(100.0 / batch_size).item())
            print('-----------------------------------')
