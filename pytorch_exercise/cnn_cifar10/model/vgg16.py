...
class vgg_net(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            ...
            nn.Conv2d(128, 128, 3, padding=1), nn.ReLU(True), # Conv7
            nn.MaxPool2d(2, 2),  # Pool3

            nn.Conv2d(128, 256, 3, padding=1), nn.ReLU(True), # Conv8
            nn.Conv2d(256, 256, 3, padding=1), nn.ReLU(True), # Conv9
            nn.Conv2d(256, 256, 3, padding=1), nn.ReLU(True), # Conv10
            nn.MaxPool2d(2, 2),  # Pool4

            nn.Conv2d(256, 256, 3, padding=1), nn.ReLU(True), # Conv11
            nn.Conv2d(256, 256, 3, padding=1), nn.ReLU(True), # Conv12
            nn.Conv2d(256, 256, 3, padding=1), nn.ReLU(True), # Conv13
        ...
        # add weight decay(L2), 
        self.optimizer = optim.SGD(self.parameters(), lr = 1e-2, momentum = 0.9, weight_decay=0.001)
