...
class parallel_net(nn.Module):
    def __init__(self, conv_layers, recover_mode = 'W', interpolation = True, device = None):
        ...
        self.latest_train_cam = torch.zeros((50000, 1, 8, 8), dtype=torch.float32, requires_grad=False).to(device)
        self.latest_valid_cam = torch.zeros((10000, 1, 8, 8), dtype=torch.float32, requires_grad=False).to(device)
